...
class UserContent:
    ...
    def __getitem_with_row__(s, i):
        ...
        item = s.content.iloc[
            i
        ]  # Returns a comma separated data frame from the CSV file.
        return item
    ...
    def __get_usr_id__(s):
        return s.usr_id

    def __get_language__(s):
        return s.language

    def __get_oldest_item__(s):
        return s.oldest_item

    def __get_latest_item__(s):
        return s.latest_item

    def __get_filename__(s):
        return s.filename
    ...
    def __set_language__(s, language):
        s.language = language
    ...
    def usr_followers(s):
        ...
        if not isinstance(s.__get_user_item__()["following"], str):
            return usr_fol_id
        fol_list = s.__get_user_item__()["following"].translate(
    ...
    def content_request(s, n_items=0, all_terms=0):
        ...
        if not (all_terms == 0 or all_terms == 1):
            print(
                "all_terms takes invalid value, should be 0 or 1 but instead is "
                + str(all_terms)
            )
        ...
        if n_items == 0:
            ...
            s.content["created"] = pd.to_datetime(
                s.content["created"], format="%Y-%m-%d %H:%M:%S.%f"
            )
            ...
            for c in range(r1, r2):
                ...
                if prior:
                    content_return_pr, valid = s.item_valid(
                        item_id,
                        content_return_pr,
                        times,
                        langcheck,
                    ...
                    content_return, valid = s.item_valid(
    ...
    def apprv_content(s, all_terms=0):
        ...
        times = [
            s.content["created"].min(skipna=True),
            s.content["created"].max(skipna=True),
        ]
        usr_fol_list = s.usr_followers()
        for item_id in s.content.index:
            ...
            if prior:
                content_return_pr, valid = s.item_valid(
                    item_id, content_return_pr, times, langcheck, languages, all_terms
                )
            else:
                content_return, valid = s.item_valid(
                    item_id, content_return, times, langcheck, languages, all_terms
