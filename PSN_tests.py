...
def test4():
    ...
    for i in range(nAve):
        ...
        if appr_test(output_df):
            ...
            print(
                "Test failed for n_items None and latest_item in row %d" % rand_lat_num
            )
            return False
    return True
...
def test4b():
    ...
    for i in range(nAve):
        ...
        if appr_test(output_df):
            ...
            print(
                "Test failed for n_items rand %d and latest_item rand in row %d"
                % (n_items, rand_lat_num)
            )
            return False
    return True
...
def test11():
    ...
    if not appr_test(output_df):
        print(
            "Test failed to detect both oldest "
            + str(UC.oldest_item)
            + " and latest item "
            + str(UC.latest_item)
            + "were set"
...
def test12():
    ...
    for i in range(nAve):
        ...
        if appr_test(output_df):
            ...
            print(
                "Test failed for n_items None and oldest_item in row %d" % rand_lat_num
            )
            return False
    return True
...
def test12b():
    ...
    for i in range(nAve):
        ...
        if appr_test(output_df) and shape_test(output_df, n_items):
            ...
            print(
                "Test failed for n_items rand %d and oldest_item rand in row %d"
                % (n_items, rand_lat_num)
            )
            return False
    return True
...
def test14():
    ...
    last_time = pd.to_datetime(
        content.iloc[rand_lat_num]["created"], format="%Y-%m-%d %H:%M:%S.%f"
    )
    latest_item = content.index[rand_lat_num]
    ...
    first_time = pd.to_datetime(
        output_df["created"].max(), format="%Y-%m-%d %H:%M:%S.%f"
    )
    content["created"] = pd.to_datetime(
        content["created"], format="%Y-%m-%d %H:%M:%S.%f"
    )
    times = [
        content["created"].min() - pd.Timedelta(days=1),
        content["created"].max() + pd.Timedelta(days=1),
    ]
...
def test15():
    ...
    user_id = (
        "6xrOHj0UG9R0Zb06MYUxQ2F7cqv2"  # User with language settings but no following
    )
...
def test16_pivot():
    ...
    cimpossible = [
        ...
        if pd.to_datetime(content.loc[c]["created"], format="%Y-%m-%d %H:%M:%S.%f")
        > today
    ]
    ...
    if (
        piv1 in oldest_item
        and first_time == content["created"].min()
        and last_time == content.loc[oldest_item]["created"]
        and not assbool
    ):
        test = True
    else:
        ...
        print(
            ...
            + str(first_time)
            + " and should be "
            + str(content["created"].min())
        )
        print(
            "last_time is "
            + str(last_time)
            + " and should be "
            + str(content.loc[oldest_item]["created"])
        )
    ...
    piv1, last_time, first_time, assbool = UC.pivot_terms()
    if (
        piv1 in content.index[0]
    ):  # and abs((last_time-pd.Timestamp.today()).seconds)<60**2 and abs((first_time-content.loc[latest_item]['created']).seconds)<60 and not assbool:
        test = True
    else:
        print(
            "Pivot test failed for latest_item defined and oldest_item None and latest_item more than a day old"
        )
        ...
        print(
            ...
            + str(first_time)
            + " and should be "
            + str(content.loc[latest_item]["created"])
        )
        print("sorting bool is " + str(assbool) + " and should be False")
        return False
    ...
    if (
        piv1 in content.index[0]
        and last_time == content["created"].max()
        and first_time == content["created"].min()
        and not assbool
    ):
        test = True
        ...
        print(
            "Pivot test failed for latest_item defined and oldest_item None and latest_item less than a day old"
        )
        print("Pivot should be " + content.index[0] + " and is " + piv1)
        print(
            "last_time is " + last_time + " and should be " + content["created"].max()
        )
        print(
            "first_time is " + first_time + " and should be " + content["created"].min()
        )
        print("sorting bool is " + str(assbool) + " and should be False")
        return False
    ...
    oldest_item = None
    UC = PSN_CodeChallenge.UserContent(
        csv_filename=csvfilename, latest_item=latest_item, oldest_item=oldest_item
    )
    piv1, last_time, first_time, assbool = UC.pivot_terms()
    if (
        piv1 in latest_item
        and abs((first_time - content.loc[latest_item]["created"]).seconds) < 2
        and abs((last_time - today).seconds) < 60
        and assbool
    ):
        test = True
    else:
...
def test17_itemvalid_anytime():
    ...
    content_return, test = UC.item_valid(
        item_id, content_return, times, langcheck, languages, all_terms
    )
    if not test:
    ...
    if not test:
        print(
            "item_valid test valid for language check on and valid language and item APPROVED"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check on and invalid language and item APPROVED"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check on and valid language and item REJECTED"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check on and invalid language and item REJECTED"
        )
        return False
...
def test17_item_valid_validtime():
    ...
    if not test:
        print(
            "item_valid test valid for language check off and item APPROVED, and time in interval"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check off and item REJECTED, and time in interval"
        )
        return False
    ...
    if not test:
        print(
            "item_valid test valid for language check on and valid language and item APPROVED and in time interval"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check on and invalid language and item APPROVED and in time interval"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check on and valid language and item REJECTED and in valid time interval"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check on and invalid language and item REJECTED and in valid time interval"
        )
        return False
...
def test17_itemvalid_outsidetime():
    ...
    if test:
        print(
            "item_valid test valid for language check off and item APPROVED but time too late"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check off and item REJECTED,time too late"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check off and item PENDING, time too late"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check on and valid language and item APPROVED, time too late"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check on and invalid language and item APPROVED, time too late"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check on and valid language and item REJECTED, time too late"
        )
        return False
    ...
    if test:
        print(
            "item_valid test valid for language check on and invalid language and item REJECTED, time too late"
        )
        return False
    # langcheck on, item PENDING, valid language
    item_id = "600f87ac06640000183c1234"
...
def shape_test(output_df, n_items):
    ...
    if len(output_df.shape) <= 1 and output_df.shape[0] == df_cols:
        ...
        errstr += ", shape is (%d,%d) and should be (%d,%d)" % (
            output_df.shape[0],
            output_df.shape[1],
            n_items,
            df_cols,
