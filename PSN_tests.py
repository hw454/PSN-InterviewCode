...
test4: For random "latest_item" test all content terms are APPROVED.
...
def test4(UC):
  ...
  for i in range(nAve):
    ...
    if appr_test(output_df) and output_df.shape[0]<=n_items:
      continue
    else:
...
def test5b(UC):
  ...
  if  appr_test(test5out) and test5out==apprv_df:
    return True
  else:
    print('test for n_items negative failed')
    return False
...
def test6(UC):
  ...
  apprv_df=UC.apprv_content()
  if  appr_test(test6out) and apprv_df==test6out:
    return True
  else:
